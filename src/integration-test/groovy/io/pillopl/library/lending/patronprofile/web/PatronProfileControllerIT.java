...
public class PatronProfileControllerIT {
  ...
  @MockitoBean private PatronProfiles patronProfiles;

  @MockitoBean private PlacingOnHold placingOnHold;

  @MockitoBean private CancelingHold cancelingHold;
  ...
  public void shouldReturnPatronProfileSummaryWithCountsAndLinks() throws Exception {
    ...
    given(patronProfiles.fetchFor(patronId)).willReturn(profileWithCurrentActivity());

    mvc.perform(get("/profiles/" + patronId.getPatronId()).accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        ...
        .andExpect(
            jsonPath("$._links.self.href", containsString("/profiles/" + patronId.getPatronId())))
        .andExpect(
            jsonPath(
                "$._links.holds.href",
                containsString("/profiles/" + patronId.getPatronId() + "/holds")))
        .andExpect(
            jsonPath(
                "$._links.checkouts.href",
                containsString("/profiles/" + patronId.getPatronId() + "/checkouts")));
  }

  @Test
  public void shouldReturnZeroCountsWhenPatronHasNoCurrentActivity() throws Exception {

    given(patronProfiles.fetchFor(patronId)).willReturn(emptyProfile());
  ...
  @Test
  public void shouldCountOnlyOverdueCurrentCheckouts() throws Exception {
    ...
    PatronProfile profile =
        new PatronProfile(
            ...
            new CheckoutsView(
                of(new Checkout(overdueBook, overdueDate), new Checkout(futureBook, futureDate))));

    given(patronProfiles.fetchFor(patronId)).willReturn(profile);
  ...
  @Test
  public void shouldReturnEmptySummaryForUnknownPatron() throws Exception {

    PatronId unknownPatronId = PatronFixture.anyPatronId();

    given(patronProfiles.fetchFor(unknownPatronId)).willReturn(emptyProfile());

    mvc.perform(
            get("/profiles/" + unknownPatronId.getPatronId())
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.patronId", is(unknownPatronId.getPatronId().toString())))
        .andExpect(jsonPath("$.currentHoldsCount", is(0)))
        .andExpect(jsonPath("$.currentCheckoutsCount", is(0)))
        .andExpect(jsonPath("$.overdueCheckoutsCount", is(0)))
        .andExpect(
            jsonPath(
                "$._links.holds.href",
                containsString("/profiles/" + unknownPatronId.getPatronId() + "/holds")))
        .andExpect(
            jsonPath(
                "$._links.checkouts.href",
                containsString("/profiles/" + unknownPatronId.getPatronId() + "/checkouts")));
  }
  ...
  public void shouldCreateLinksForHolds() throws Exception {
    ...
    mvc.perform(
            get("/profiles/" + patronId.getPatronId() + "/holds/")
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isOk())
        .andExpect(header().string(CONTENT_TYPE, MediaTypes.HAL_FORMS_JSON_VALUE))
        ...
        .andExpect(
            jsonPath(
                ...
                containsString(
                    "/profiles/" + patronId.getPatronId() + "/holds/" + bookId.getBookId())))
  ...
  public void shouldCreateLinksForCheckouts() throws Exception {
    ...
    mvc.perform(
            get("/profiles/" + patronId.getPatronId() + "/checkouts/")
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isOk())
        .andExpect(header().string(CONTENT_TYPE, MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(
            jsonPath("$._embedded.checkoutList[0].bookId", is(anotherBook.getBookId().toString())))
  ...
  public void shouldReturn404WhenThereIsNoHold() throws Exception {
    ...
    String path = "/profiles/" + patronId.getPatronId() + "/holds/" + UUID.randomUUID();

    mvc.perform(get(path).accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code", is("HOLD_NOT_FOUND")))
        .andExpect(jsonPath("$.message", is("The requested hold was not found.")))
  ...
  public void shouldReturn404WhenThereIsNoCheckout() throws Exception {
    ...
    String path = "/profiles/" + patronId.getPatronId() + "/checkouts/" + UUID.randomUUID();

    mvc.perform(get(path).accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code", is("CHECKOUT_NOT_FOUND")))
        .andExpect(jsonPath("$.message", is("The requested checkout was not found.")))
        .andExpect(jsonPath("$.path", is(path)))
        .andExpect(jsonPath("$.timestamp").exists())
        .andExpect(jsonPath("$.details").isArray());
  }
  ...
  public void shouldReturnResourceForHold() throws Exception {
    ...
    mvc.perform(
            get("/profiles/" + patronId.getPatronId() + "/holds/" + bookId.getBookId())
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isOk())
        .andExpect(header().string(CONTENT_TYPE, MediaTypes.HAL_FORMS_JSON_VALUE))
  ...
  public void shouldReturnResourceForCheckout() throws Exception {
    ...
    mvc.perform(
            get("/profiles/" + patronId.getPatronId() + "/checkouts/" + anotherBook.getBookId())
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isOk())
        .andExpect(header().string(CONTENT_TYPE, MediaTypes.HAL_FORMS_JSON_VALUE))
  ...
  public void shouldReturn500IfSomethingFailedWhileDuringPlacingOnHold() throws Exception {
    ...
    given(placingOnHold.placeOnHold(any())).willThrow(new IllegalArgumentException());
    ...
    mvc.perform(
            post("/profiles/" + patronId.getPatronId() + "/holds")
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE)
  ...
  public void shouldCancelExistingHold() throws Exception {
    ...
    given(cancelingHold.cancelHold(any())).willReturn(Success);

    mvc.perform(
            delete("/profiles/" + patronId.getPatronId() + "/holds/" + bookId.getBookId())
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE))
  ...
  public void shouldNotCancelNotExistingHold() throws Exception {
    ...
    given(cancelingHold.cancelHold(any())).willThrow(new HoldNotFoundException(bookId));

    mvc.perform(delete(holdPath()).accept(MediaTypes.HAL_FORMS_JSON_VALUE))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code", is("HOLD_NOT_FOUND")))
        .andExpect(jsonPath("$.message", is("The requested hold was not found.")));
  }
  ...
  public void shouldReturn500IfSomethingFailedWhileCanceling() throws Exception {
    ...
    given(cancelingHold.cancelHold(any())).willThrow(new IllegalStateException());
  ...
  public void shouldReturn404WhenBookDoesNotExist() throws Exception {
    ...
    given(placingOnHold.placeOnHold(any())).willThrow(new BookNotFoundException(bookId));
    ...
    mvc.perform(
        ...
        .andExpect(jsonPath("$.message", is("The requested book was not found.")))
        .andExpect(jsonPath("$.details").isArray());
  }

  @Test
  public void shouldReturn404WhenPatronDoesNotExist() throws Exception {

    given(placingOnHold.placeOnHold(any())).willThrow(new PatronNotFoundException(patronId));
    ...
    mvc.perform(
            post(placeHoldPath())
                ...
                .content(validPlaceHoldRequest()))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code", is("PATRON_NOT_FOUND")))
        .andExpect(jsonPath("$.message", is("The requested patron was not found.")));
  }

  @Test
  public void shouldReturn404WhenCancelingUnknownHold() throws Exception {
  ...
  @Test
  public void shouldReturn409WhenHoldCancellationIsRejected() throws Exception {

    given(cancelingHold.cancelHold(any())).willReturn(Rejection);

    mvc.perform(delete(holdPath()))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.code", is("HOLD_CANCELLATION_NOT_ALLOWED")))
        .andExpect(jsonPath("$.message", is("The hold cannot be canceled in its current state.")));
  }

  @Test
  public void shouldReturnGeneric500WhenCancelingUnexpectedlyFails() throws Exception {

    given(cancelingHold.cancelHold(any()))
        .willThrow(new IllegalStateException("Internal implementation detail"));
  ...
  private PatronProfile emptyProfile() {
    return new PatronProfile(PatronStatus.ACTIVE, new HoldsView(of()), new CheckoutsView(of()));
  }
  ...
  public void shouldPlaceBookOnHold() throws Exception {
    given(placingOnHold.placeOnHold(any())).willReturn(Success);
    mvc.perform(
            post(placeHoldPath())
                .accept(MediaTypes.HAL_FORMS_JSON_VALUE)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  public void shouldAllowMissingNumberOfDays() throws Exception {
    ...
    String request =
        "{"
            ...
            + "\"a518e2ef-5f6c-43e3-a7fc-5d895e15be3a\""
            + "}";
    mvc.perform(post(placeHoldPath()).contentType(MediaType.APPLICATION_JSON).content(request))
        .andExpect(status().isOk());
  }
  ...
  @Test
  public void shouldReturn400WhenLibraryBranchIdIsMissing() throws Exception {

    String request =
        "{" + "\"bookId\":\"6e1dfec5-5cfe-487e-814e-d70114f5396e\"," + "\"numberOfDays\":1" + "}";

    mvc.perform(post(placeHoldPath()).contentType(MediaType.APPLICATION_JSON).content(request))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is("VALIDATION_FAILED")))
        .andExpect(jsonPath("$.details[0].field", is("libraryBranchId")));
  }

  @Test
  public void shouldReturn400WhenNumberOfDaysIsZero() throws Exception {
    ...
    mvc.perform(
        ...
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is("VALIDATION_FAILED")))
        .andExpect(jsonPath("$.details[0].field", is("numberOfDays")));
  }

  @Test
  public void shouldReturn400WhenNumberOfDaysIsNegative() throws Exception {
  ...
  @Test
  public void shouldReturn400ForMalformedRequestBody() throws Exception {
    ...
    mvc.perform(post(placeHoldPath()).contentType(MediaType.APPLICATION_JSON).content(request))
        ...
        .andExpect(jsonPath("$.message", is("The request body is malformed.")))
        .andExpect(jsonPath("$.details").isArray());
  }
  ...
  @Test
  public void shouldReturn409WhenHoldIsRejectedByDomainPolicy() throws Exception {

    given(placingOnHold.placeOnHold(any())).willReturn(Rejection);
    mvc.perform(
            post(placeHoldPath())
                .contentType(MediaType.APPLICATION_JSON)
                .content(validPlaceHoldRequest()))
