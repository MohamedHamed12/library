...
public class PatronControllerIT {
  ...
  @MockitoBean private RegisteringPatron registeringPatron;

  @MockitoBean private SuspendingPatron suspendingPatron;

  @MockitoBean private ReactivatingPatron reactivatingPatron;
  ...
  public void shouldRegisterRegularPatronAndReturn201WithLocation() throws Exception {
    given(registeringPatron.register(any())).willReturn(patronId);

    String request = "{" + "\"type\":\"Regular\"," + "\"email\":\"regular@example.test\"" + "}";

    mvc.perform(post("/patrons").contentType(MediaType.APPLICATION_JSON).content(request))
        .andExpect(status().isCreated())
        .andExpect(
            header().string("Location", containsString("/profiles/" + patronId.getPatronId())));
  }
  ...
  public void shouldRegisterResearcherPatronAndReturn201WithLocation() throws Exception {
    ...
    String request =
        "{" + "\"type\":\"Researcher\"," + "\"email\":\"researcher@example.test\"" + "}";
  ...
  public void shouldReturn400WhenTypeIsMissing() throws Exception {
    String request = "{" + "\"email\":\"nobody@example.test\"" + "}";
  ...
  public void shouldReturn400WhenEmailIsInvalid() throws Exception {
    ...
    String request = "{" + "\"type\":\"Regular\"," + "\"email\":\"not-an-email\"" + "}";
  ...
  public void shouldReturn409WhenEmailIsAlreadyRegistered() throws Exception {
    ...
    String request = "{" + "\"type\":\"Regular\"," + "\"email\":\"duplicate@example.test\"" + "}";
  ...
  public void shouldReturn400WhenTypeIsInvalid() throws Exception {
    String request = "{" + "\"type\":\"INVALID\"," + "\"email\":\"nobody@example.test\"" + "}";
