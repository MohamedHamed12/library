...
public class PatronController {
  ...
  public ResponseEntity<Void> registerPatron(
    ...
    PatronId patronId =
        registeringPatron.register(
            new RegisterPatronCommand(
                Instant.now(clock), request.getType(), EmailAddress.of(request.getEmail())));

    return ResponseEntity.created(
            uriComponentsBuilder
                .path("/profiles/{patronId}")
                .buildAndExpand(patronId.getPatronId())
                .toUri())
        .build();
  }
  ...
  public ResponseEntity<Void> suspend(
      @PathVariable UUID patronId, @Valid @RequestBody SuspendPatronRequest request) {
    SuspendPatronCommand command =
        new SuspendPatronCommand(Instant.now(clock), new PatronId(patronId), request.getReason());

    Result result = suspendingPatron.suspend(command);

    rejectIfNeeded(
        result, ApiErrorCode.PATRON_ALREADY_SUSPENDED, "The patron is already suspended.");
  ...
  public ResponseEntity<Void> reactivate(@PathVariable UUID patronId) {
    ReactivatePatronCommand command =
        new ReactivatePatronCommand(Instant.now(clock), new PatronId(patronId));

    Result result = reactivatingPatron.reactivate(command);

    rejectIfNeeded(result, ApiErrorCode.PATRON_ALREADY_ACTIVE, "The patron is already active.");
